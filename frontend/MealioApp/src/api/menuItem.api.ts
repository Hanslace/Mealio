...
export const getMenuItemsByRestaurant = async (restaurantId: number) => {
  const res = await axiosInstance.get(`/menu-item/restaurant/${restaurantId}`);
...
export const updateMenuItem = async (
  ...
  const res = await axiosInstance.put(`/menu-item/${itemId}`, data);
  return res.data;
};

// ✅ Delete a menu item
export const deleteMenuItem = async (itemId: number) => {
  const res = await axiosInstance.delete(`/menu-item/${itemId}`);
  return res.data;
};

// ✅ Get most liked menu items (Top 10 globally)
export const getMostLikedItems = async () => {
  const res = await axiosInstance.get('/menu-item/most-liked');
...
export const getTrendingItems = async (days = 7) => {
  const res = await axiosInstance.get(`/menu-item/trending?days=${days}`);
