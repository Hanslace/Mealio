...
/**
 * Admin: list all delivery personnel
 */
module.exports.listAll = async (req, res, next) => {
  try {
    const profiles = await DeliveryPersonnel.findAll({
      include: {
        model: User,
...
module.exports.verifyProfile = async (req, res, next) => {
  try {
    ...
    const profile = await DeliveryPersonnel.findByPk(delivery_id);
    if (!profile) return res.status(404).json({ error: 'Profile not found' });

    profile.verification_status = status;
    await profile.save();

    return res.json({ message: 'Verification status updated', profile });
  } catch (err) {
    next(err);
  }
};
